...
#define DISP 100
/*
 * the bus is the initial value of offset for rank 1. It should start from 0 (same as rank 0) because both rank 0 and
 * rank 1 do the Isend and Irecv.
 * To fix: I change offset = 0 for rank 1.
 * The second bug is that rank 2 does not non-blocking send/recv, therefore it does not need MPI_Waitall.
 * To fix: I put MPI_Waitall to only rank 0, 1 and rank 3
 */
int main (int argc, char *argv[])
{
    int numtasks, rank, buf, tag1=1,tag2=2, i, rc, dest, src, offset, nreqs;
    double T1, T2;
