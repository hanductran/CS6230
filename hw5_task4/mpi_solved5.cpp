...
#define MSGSIZE 2000
/*
 * The bug is the declaration of MPI_BYTE in send and receive comments. Because data is an array of characters, we should
 * use MPI_CHAR (instead of MPI_BYTE) to be safe. A character is usually defined as 1 byte. However, for different platforms
 * the character could be defined as different number of bytes. That is the unsafe point of the code.
 * To fix: replace MPI_BYTE by MPI_CHAR in send and receive commands.
 */
